...
public class NazioneDAO extends AbstractElasticSearchDAO<Nazione> implements INazioneDAO {
    ...
    public IPageResult<Nazione> findNazioneByQuery(INazioneSearchBuilder nazioneSearchBuilder) throws Exception {
        ...
        return super.find(new MultiFieldsSearch(nazioneSearchBuilder.getFieldSort(), SortOrder.ASC,nazioneSearchBuilder.getFrom(),size,nazioneSearchBuilder.getListAbstractBooleanSearch().toArray(new IBooleanSearch[nazioneSearchBuilder.getListAbstractBooleanSearch().size()])));
    }
