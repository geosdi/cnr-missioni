...
public interface IPrenotazioneForm extends IForm<PrenotazioneEvent, IPrenotazioneForm> {
	...
	class PrenotazioneForm extends IForm.FormAbstract<PrenotazioneEvent, IPrenotazioneForm>
		...
		public void buildTab() {
			...
			if (modifica)
				try {
					User user = ClientConnector.getUser(
							IUserSearchBuilder.UserSearchBuilder.getUserSearchBuilder().withId(bean.getIdUser()))
							.getUsers().get(0);
					userLabel = new TextField("User", user.getAnagrafica().getCognome().concat(" ").concat(user.getAnagrafica().getNome()));
